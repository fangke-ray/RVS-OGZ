...
$(function(){
	$("#add_rc_mailsend_date,#add_rc_ship_assign_date").datepicker({
		showButtonPanel : true,
		dateFormat : "yy/mm/dd",
		currentText : "今天"
	});
	$("#add_service_repair_flg,#add_search_service_free_flg,#add_workshop").select2Buttons();
  ...
  $("#ins_serviceRepairManage").validate({
	...
	var getStepIntoPcsComment = function(updatedata){
		...
		if ($steps.length > 0) {
			...
			return '{"GC6210100":"'+retString.replace("\\","")+'"}';
		} else {
			return null;
		}
	}
...
var show_analysis_Complete = function(xhrobj, textStatus) {// 点击分析button触发Dialog
    var linkna = "widgets/service_repair_referee_analysis.jsp";//load 页面   
...
function service_repair_list(listdata) {
	if ($("#gbox_list").length > 0) {
		...
		$("#uld_list").jqGrid({
			...
			shrinkToFit:true,
			datatype : "local",  
			colNames : [ '', '型号', '机身号', '修理单号', '类别', 'RC邮件发送日', 'RC出货指示日', 'SORC受理日'],
			colModel : [ {
				name : 'material_id',
				index : 'material_id',
				// hidden:true,
