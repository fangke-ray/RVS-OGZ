	...
	$.ajax({
		...
		complete : function(xhrObj){
			...
			if (resInfo.check_sheet) {
				...
				$check_sheet.find("input.ui-button,button.t_pic").button();
				$check_sheet.find("button.t_pic").click(function(){
					var $check_picture = $("#c_picture");

					$check_picture.html("<img src='http://" + document.location.hostname + "/photos/jig/" 
						+ $(this).closest("tr").children("td:eq(2)").text().trim() + "' title='点击图片关闭'>");
			...
			if(isLeader == "true") {
				...
				}).filter(":text").bind("drop", function(idx, ele) {
					return false;
				});; // TODO drop?
						...
						if (object_type == 2) {
							...
							$check_sheet.find("tr[stat=change]").each(function() {
								...
								$input.each(function(idx, ele){
									...
									if (inputVal > 999999.999) {
										inputVal = 999999.999; ele.value = "∞";
									} else if (inputVal < -999999.999) {
										inputVal = -999999.999; ele.value = "∞";
									}
						...
						if (!noNan) {
							...
							if (object_type == 2) {
								...
								if (infectDetailData.check_file_manage_id == "00000000305"
									...
									if ($dtag.length > 1) {
										var manageIdSet = {};
										$check_sheet.find("tr[stat=change] td.tcs_input[status=2] input").each(function(){
											var dManageId = this.name.split("_")[1];
											if (!(dManageId in manageIdSet)) {
												manageIdSet[dManageId] = "1";
											}
										});
