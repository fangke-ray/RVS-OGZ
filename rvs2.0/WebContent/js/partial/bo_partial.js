...
function initGrid() {
		$("#list").jqGrid({
			...
			rowheight : 23,
			datatype : "local",
			colNames : ['维修对象ID','修理单号', '订单次数', '型号 ID', '型号', '机身号', '等级', '纳期', '零件订购日', '零件到货','入库预定日', '订单状态', '零件缺品详细', '加急', '发生工位' ,'进展工位'],
			colModel : [{
						name: 'material_id',
						index: 'material_id',
						hidden : true
						...
						align : 'center',
						formatter:'select',
						editoptions:{value:$("#goPartialBoFlg").val()}
					}, {
						name : 'bo_contents',
						index : 'bo_contents',
						width : 150,
...
var findit = function(data) {
	if (!data) { //新查询
		keepSearchData = {
			...
			"has_bo_content":$("#search_has_bo_content").attr("checked") == "checked" ? $("#search_has_bo_content").val() : 0
		};
	} else {
		keepSearchData = data;
	}
...
function doOk(material_id,materialPartialFormList,occur_times){
	var materialPartial = "";
	for(var i = 0; i< materialPartialFormList.length;i++){
		materialPartial = materialPartialFormList[i];
		var curValue = materialPartial.occur_times;
		if(curValue == occur_times) break;
	}
	
	
	//零件缺品备注
	var bo_contents = {};
	$("#material_detail_paticalarea textarea").each(function(index,ele){
		var $textarea = $(ele);
		var value = $textarea.val().trim();
		if(value){
			bo_contents[$textarea.attr("line_id")] = $textarea.val();
		}
	});
	// 无BO零件时
	if ($.isEmptyObject(bo_contents)) bo_contents = null;

	var partial = {
		"material_id" : material_id,
		"occur_times":$("#edit_occur_times").val(),
		"bo_position":$("#edit_bo_position").val(),
		...
		"bo_contents":Json_to_String(bo_contents)
	};
	
	function checkChange(materialPartial,partial){
		if(!compare(materialPartial.bo_position,partial.bo_position)) return true;
		if(!compare(materialPartial.bo_flg,partial.bo_flg)) return true;
		if(!compare(materialPartial.order_date,partial.order_date)) return true;
		if(!compare(materialPartial.arrival_plan_date,partial.arrival_plan_date)) return true;
		if(!compare(materialPartial.arrival_date,partial.arrival_date)) return true;
		if(!compare(materialPartial.bo_contents,partial.bo_contents)) return true;
	...
	if(checkChange(materialPartial,partial)){
		if(partial.bo_flg == 9 && (partial.order_date == null || partial.order_date == "")){//订单状态为待发放情况下，零件订购日没有填写则弹出警告信息
			warningConfirm("已经订购的情况下，需要零件订购日，请输入零件订购日的值。",function(){
				$.ajax({
					beforeSend : ajaxRequestType,
					async : true,
					url : "materialPartial.do?method=doUpdate",
					cache : false,
					data : partial,
					type : "post",
					dataType : "json",
...
var uploadfile = function() {
	...
	$.ajaxFileUpload({
		url : 'upload.do?method=doPartOrder', // 需要链接到服务器地址
		secureuri : false,
...
function initView(){
	...
	$("#resetbutton").click(function() {
		...
		$("#search_has_bo_content").attr("checked",false).trigger("change");
	});
	$("#search_range").val("1").trigger("change");
	...
	$("#reportbobutton").click(function(){
		reportMaterialPartial(servicePath);
	});
	...
	$("#uploadtogglebutton").click(function(){$("#uploadarea").toggle()});
	...
	$("#loginInOrderbutton").click(order);
	
	function order(){
		$.ajax({
			beforeSend : ajaxRequestType,
			async : true,
			url : servicePath + '?method=searchNotOrderMaterail',
			cache : false,
			data : null,
			type : "post",
			dataType : "json",
			success : ajaxSuccessCheck,
			error : ajaxError,
			complete : searchNotOrderMaterailComplete
		});
	};

	var searchNotOrderMaterailComplete = function(xhrobj, textStatus) {
		var resInfo = null;
		try {
			// 以Object形式读取JSON
			eval('resInfo =' + xhrobj.responseText);
	...
	//所有没有投线，也没有建立过维修对象零件订购单的维修对象
	function notOrderList(list){
		if ($("#gbox_orderlist").length > 0) {
			$("#orderlist").jqGrid().clearGridData();
			$("#orderlist").jqGrid('setGridParam',{data:list}).trigger("reloadGrid", [{current:false}]);
		} else {
			$("#orderlist").jqGrid({
				data:list,
				height: 461,
				...
				rowheight: 23,
				datatype: "local",
				colNames:['','修理单号','型号名称','机身号','等级','受理时间','同意日'],
				colModel:[
					{name :'material_id',index:'material_id', hidden:true},		   
					{name :'omr_notifi_no',index :'omr_notifi_no',width : 120},
					{name :'model_name',index :'model_name',width : 100},
					{name :'serial_no',index :'serial_no',width : 100},
					{name :'level',index:'level',width:60,align:'center', formatter:'select', editoptions:{value:$("#goMaterialLevel").val()} },
					{name :'reception_time',index:'reception_time',width:100,align:'center',formatter:'date',formatoptions:{srcformat:'Y/m/d H:i:s',newformat:'Y-m-d'}},
					{name :'agreed_date',index:'agreed_date',width:100,align:'center',formatter:'date',formatoptions:{srcformat:'Y/m/d',newformat:'Y-m-d'}}		   
				 ],
				rowNum: 100,
				toppager: false,
				...
				recordpos: 'left',
				ondblClickRow : function(rid, iRow, iCol, e) {
					var rowData = $("#orderlist").getRowData(rid);
					materialPartialDetail(rowData);
				},
				viewsortcols : [true,'vertical',true],
	...
	var materialPartialDetail = function(rowData){
		
		$("#materialPartialDetail").load(servicePath + "?method=detail",function(responseText, textStatus, XMLHttpRequest){
			
			$("#material_detail_paticalarea > div").eq(0).remove();
			
			$("#insert_div_no_arrival_plan_date").buttonset();
			$("#insert_arrival_plan_date").datepicker({
				showButtonPanel:true,
				dateFormat: "yy/mm/dd",
				currentText: "今天"
			});
			...
			$("#insert_no_arrival_plan_date").click(function(){
				if ($(this).attr("checked")) {
					$("#insert_arrival_plan_date").hide().val("9999/12/31");
				} else {
					$("#insert_arrival_plan_date").show().val("");
				}
			});
				
			$(this).dialog({
				title : "修理单号为:" + rowData.omr_notifi_no + "的维修对象零件订购信息",
				width : 800,
				show : "blind",
				height : 'auto' ,
				resizable : false,
				...
				buttons : {
					"确定":function(){
						...
						var dada = {
							...
							"bo_flg" : "7",//预提
							"occur_times" : "1",
							"bo_position" : $("#insert_bo_position").val(),
							"arrival_plan_date": $("#insert_arrival_plan_date").val(),
							"bo_contents" : Json_to_String(bo_contents)
						};
						$.ajax({
							beforeSend : ajaxRequestType,
			...
			var doInsertComplete = function(xhrobj, textStatus) {
				var resInfo = null;
				try {
					// 以Object形式读取JSON
					eval('resInfo =' + xhrobj.responseText);
					if (resInfo.errors.length > 0) {
						...
						$("#materialPartialDetail").dialog('close');
						order();
						findit();
					}
				} catch (e) {
...
//报表导出
var reportMaterialPartial = function(servicePath){
	$.ajax({
		beforeSend: ajaxRequestType, 
		async: true, 
