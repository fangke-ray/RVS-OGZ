		...
		$("#exd_list").jqGrid({
			...
			gridComplete: function(){				
				...
				if($("#count_result").length==0){
					$("#exd_listarea .ui-jqgrid-titlebar").append("<span id='count_result' style='float:right;margin-right:6px;font-size:15px;'>当前检索范围内，" +
							"检查总数"+records+"台；" +
							"不合格"+j+"台；检查合格率"+c+"%</span>");
				} else {
					$("#count_result").text("当前检索范围内，检查总数"+records+"台；" +
							"不合格"+j+"台；检查合格率"+c+"%").attr("font-size","15px");
				}
		}
	});	
   }
