...
$(function() {
	...
	$("#worktimecount").click(function(){
		$.ajax({
			beforeSend : ajaxRequestType,
			async : false,
			url : servicePath + '?method=worktimeExport',
			cache : false,
			data : keepSearchData,
			type : "post",
			dataType : "json",
			...
			complete : function(xhjObject) {
				...
				} else if (resInfo && resInfo.filePath) {
					if ($("iframe").length > 0) {
...
function search_handleComplete(xhrobj, textStatus) {
	...
	if(keepSearchData.status == 2){
		$("#worktimecount").enable();
	}else{
		$("#worktimecount").disable();
	}
};
