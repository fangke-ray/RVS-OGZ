...
var solve = function() {
	...
	if (rowid) {
		var rowdata = $("#list").jqGrid('getRowData', rowid);
		var postData = { for_solution_area_key : rowdata.for_solution_area_key };
...
var initGrid = function(){
	$("#list").jqGrid({
		...
		'等级', '直送','投线<br>日期','零件订<br>购安排','入库<br>预定日',
		'零件<br>BO', '维修课','进展<br>工位名称','纳期','总组出<br>货安排','加急','remain_days','中断信息','备注','break_back_flg','break_level'],
		colModel: [
			...
			{name:'position_name',index:'position_name', width:50, align:'center'},
	...
	$("#planned_list").jqGrid({
		...
		rowheight: 23, 
		datatype: "local", 
		colNames: [ '维修对象ID','','计划工程', '修理单号', '机种', '型号 ID', '型号', '机身号', '等级', '直送', '客户<br>同意', 'RC', '投线日期', '入库<br>预定日', '零件<br>BO', '加急', '维修课', '进展<br>工位', '6天<br>纳期', '总组出<br>货安排','remain_days'], 
		colModel: [
			{name: 'material_id', index: 'material_id', hidden: true},
