...
var doResystem=function() {
	...
	$("#wip_pop").load(linkna, function(responseText, textStatus, XMLHttpRequest) {
		...
		$("#edit_direct").val(rowdata["direct_flg"]).trigger("change");
		$("#edit_service_repair").val(rowdata["service_repair_flg"]).trigger("change");
		$("#edit_serialno").val(rowdata["serial_no"]);

		$("#edit_direct,#edit_service_repair,#edit_fix_type,#edit_ocm,#edit_level").select2Buttons();
...
var showInput=function() {
	...
	$("#wip_pop").load(linkna, function(responseText, textStatus, XMLHttpRequest) {
		...
		$(".ui-button[value='清空']").button();
		$("#edit_fix_type").next().remove();
		$("#edit_fix_type").html("<option value=\"3\" selected>检测备品</option>").select2Buttons();
		$("#wip_pop").dialog({
			position : [ 800, 20 ],
			title : "维修对象信息编辑",
			width : 400,
		...
		if (resInfo.errors.length > 0) {
			...
			if ($("#gbox_list").length > 0) {
				...
				$("#list").jqGrid({
					...
					rowheight : 23,
					datatype : "local",
					colNames : ['', '修理单号', '型号 ID', '型号', '机身号', '等级', '受理时间','返还要求','备注', 'WIP货架位置','','','','','wip_overceed'],
