...
//				ccd_operate_result = data["ccd_operate_result"];
			...
			if (data["ccd_model"] != 1 && data["ccd_model"] != 3) {
				flag_ccd = false;
			}
...
function process_dialog($process_dialog, rowdata) {
	...
	if (ref_change_flg) {
		...
		$("#ref_template").bind("change", ref_template_rc);
	}
...
$(function() {
	...
	$("#search_level, #search_fix_type, #search_direct, #search_partial_reception_flg").select2Buttons();
...
//		$("#list").trigger('reloadGrid');
	...
	$("#inlinebutton").click(function(){
		...
		for (var iDatum in srcData) {
			...
			if (srcDatum.material_id == material_id) {
				...
				img_operate_result = srcDatum.img_operate_result;
				break;
			}
		}
	...
	leverDates.nege1 = new Date($("#twoDaysBefore").val()).getTime() || 0;
	leverDates.nege2 = new Date($("#threeDaysBefore").val()).getTime() || 0;
...
var getPositions_handleComplete = function(xhrobj, textStatus) {
	...
	try {
		...
		if (resInfo.errors.length > 0) {
			...
			$("#pa_detail").flowchart({},{editable:false, selections: flowSelections});
			$("#pa_detail").html("");
		}
	} catch (e) {
		alert("name: " + e.name + " message: " + e.message + " lineNumber: "
...
function initGrid() {
	$("#list").jqGrid({
			...
			rowheight: 23,
			datatype: "local",
			colNames:['', '修理单号', '型号 ID', '型号', '类别', '机身号', '返还要求', '加急','level','等级', '报价日期', '客户同意日','同意时间',
			'零件订单', '流水分类','备注', 'WIP位置','存在画像检查','图象检查','CCD对象型号','存在CCD盖玻璃更换'
			,'CCD盖玻璃<br>/LG玻璃'
			,'维护对象ID','planned'],
			colModel:[
				...
				{name:'category_kind',index:'category_kind', width:60, hidden:true},
				...
				{name:'planned', index:'planned',hidden:true}
			],
			rowNum: 50,
			rownumbers : true,
			toppager: false,
...
//					var ccd_operate_result = rowdata["ccd_operate_result"];
					...
					var new_model = rowdata["new_model"];
					for (var iDatum in srcData) {
						var srcDatum = srcData[iDatum];
						if (srcDatum.material_id == material_id) {
							wip_date = srcDatum.inline_time;
							new_model = srcDatum.new_model;
							break;
						}
					}
					if (img_operate_result != "完成") {
					...
					if (new_model) {
						$("#list tr#" + dataIds[i] + " td[aria\\-describedby='list_model_name']").addClass("alertCell");
					}
...
//						|| (ccd_operate_result != "" && ccd_operate_result!="完成")
						...
						var levelCode = rowdata["level"];
						var vadTimestamp = new Date(vagreed_date).getTime();

						// S2/S3同意日之后两天没有投线的，同意日显示为橙色报警
						if (levelCode != 1 && vadTimestamp < leverDates.nege1)
							$("#list tr#" + dataIds[i] + " td[aria\\-describedby='list_agreed_date']").addClass("alertCell");
						// S1同意日之后三天没有投线的，同意日显示为橙色报警
						if (levelCode == 1 && vadTimestamp < leverDates.nege2)
							$("#list tr#" + dataIds[i] + " td[aria\\-describedby='list_agreed_date']").addClass("alertCell");
					}
				}
			}
		...
		$("#exe_list").jqGrid({
			...
			rowheight: 23,
			datatype: "local",
			colNames:['','投线时间', '修理单号', '型号 ID', '型号', '机身号', '等级','投入课室', '客户同意日','unrepair_flg','维护对象ID'],
			colModel:[
				{name:'myac', width:35, fixed:true, sortable:false, resize:false, formatter:'actions', formatoptions:{keys:true,delbutton:false }, hidden:true},
		...
		$.ajax({
			...
			complete : function(xhrobj, textStatus){
				...
				try {
					// 以Object形式读取JSON
					eval('resInfo =' + xhrobj.responseText);
					setLabelText(resInfo.materialForm, resInfo.materialPartialFormList, resInfo.processForm);
			...
			if (!(resInfo && resInfo.errors && resInfo.errors.length)) {
				if (resInfo.materials && resInfo.materials.length) {
					...
					for (var iM in resInfo.materials) {
						...
						tbodyText += "<tr draggable='true' material_id='" + material.material_id + "' seq='" + material.scheduled_expedited + "'";
						var clazz = "";
						...
						if (material.fix_type == 1) {
							if (isLightFix) {
								...
								if (material.inline_time) {
									...
									tbodyText += "<td pat_id='" + material.pat_id + "'><span>" 
										+ (material.operator_name || "(未设定)") + "</span><span class='ui-icon ui-icon-circle-triangle-s'></span><span class='ui-icon ui-icon-zoomin'></span></td>";
								}
							}
						}
						if (material.fix_type == 1) {
					...
					$tbodyHtml.not(".inline").bind("selectstart", function(evt){
						return false;
						...
						dataTransfer.setData("text", $tr.attr("material_id"));
						dataTransfer.setDragImage($tr.children().eq(0)[0], 0, 10);
						return true;
					}).bind("dragend", function(evt) {
						$(this).removeClass("mover");
						$inline_plan.find(".dropin").removeClass("dropin");
						// var dataTransfer = evt.dataTransfer || evt.originalEvent.dataTransfer;
						// dataTransfer.clearData("text");
						evt.preventDefault();
						return false;
					}).add($inline_plan.find("table > thead > tr")
					).bind("drop", function(evt){
						var $droper = $(this).not(".mover");
						if ($droper.length > 0) {
							$droper.removeClass("dropin");
							$mover = $inline_plan.find(".mover");

							if ($droper.children("th").length > 0) {
								var $inlinedEnd = $inline_plan.find("table tbody tr.inline").last();
								if ($inlinedEnd.length > 0) {
									$droper = $inlinedEnd;
								}
							}
							if ($droper.children("th").length > 0) {
								var startSeq = parseInt($inline_plan.find("table tbody tr").first().attr("seq"));
								changeSeq($mover, startSeq - 16);
								$inline_plan.find("table tbody").prepend($mover.detach()); // TODO inlined
							} else {
								var startSeq = parseInt($droper.attr("seq"));
								var nextSeq = $droper.next().not(".mover").attr("seq");
								if (nextSeq) {
									changeSeq($mover, parseInt((startSeq + parseInt(nextSeq)) / 2));
								} else {
									changeSeq($mover, startSeq + 16);
								}
								$droper.after($mover.detach());
							}
						...
						$inline_plan.find(".dropin").removeClass("dropin");
						$(this).not(".mover").addClass("dropin");
					})	
					;
							...
							if ($(this).hasClass("ui-icon-zoomin")) {
								...
								$("#inline_plan_selecter > select")
									...
									$.ajax({
										...
										complete : function(xhrObj) {
											var resInfo = $.parseJSON(xhrObj.responseText);
											$("#pa_detail").html("").show().flowchart("fill", resInfo.processAssigns);
											$("#inline_plan_selecter").show().css({"top" : evt.pageY, "left" : Math.min(evt.pageX, 700), "zIndex":1024, "maxWidth" : 800, "minWidth" : 200});
										}
									});
							} else {
								$("#inline_plan_selecter > select")
					...
					// $inline_plan.append($tbodyHtml.clone());
					$inline_plan
...
var changeSeq = function($target, seq) {
	var data = {material_id : $target.attr("material_id"),
	scheduled_expedited : seq};
	doChangeinlinePlan(data);
	$target.attr("seq", seq);
}
