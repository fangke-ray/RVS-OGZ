...
var showPcsRequest = function(pcs_request_key) {
	...
	$process_dialog.hide();
	// 导入编辑画面
	$process_dialog.load("widgets/pcs_request-view.html" ,
...
var showEdit = function(pcs_request_key){
	...
	$process_dialog.hide();
	// 导入编辑画面
	$process_dialog.load("widgets/pcs_request-load.html" ,
		function(responseText, textStatus, XMLHttpRequest) {
...
var importFile = function(keys){
	...
	$process_dialog.hide();
	// 导入编辑画面
	$process_dialog.load("widgets/pcs_request-import.html" ,
		function(responseText, textStatus, XMLHttpRequest) {
		var postData = {};
			...
			complete: function(xhrObj){
				...
				if (resInfo.materials.length > 0) {
					...
					$("#material_list").jqGrid({
						...
						rowheight: 23,
						datatype: "local",
						colNames: ['material_id','维修对象','型号','当前进度', 'pcs_request_key'],
						colModel: [
...
var showTest = function(pcs_request_key){
	...
	$process_dialog.hide();
	// 导入编辑画面
	$process_dialog.load("widgets/pcs_request-test.html" ,
		function(responseText, textStatus, XMLHttpRequest) {
			var detail_buttons = {
				"完成并且归档":function(){
					var postData = {
