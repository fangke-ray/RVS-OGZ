...
$(function() {
	...
	$("#input_inline_flg_set input").change(function(){
		if (this.value == 1) {
			$("#input_in_advance").enable();
		} else {
			$("#input_in_advance").disable().val("");
		}
	});
...
var showedit_handleComplete = function(xhrobj, textStatus) {
	...
	try {
		...
		if (resInfo.errors.length > 0) {
			...
			if (resInfo.lineForm.inline_flg == 1) {
				$("#input_in_advance").enable().val(resInfo.lineForm.in_advance);
			} else {
				$("#input_in_advance").disable().val("");
			}
						...
						buttons : {
							"确认" : function() {
								var data = {
									...
									"in_advance" : $("#input_in_advance").val(),
									"name" : $("#input_name").val()
								}
								$(this).dialog("close");
								// Ajax提交
