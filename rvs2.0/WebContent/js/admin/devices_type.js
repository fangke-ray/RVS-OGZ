...
$(function(){
	...
	$("input.ui-button").button();
	$("#search_specialized,#edit_specialized,#edit_hazardous_cautions").select2Buttons();
...
var showEdit = function() {
	...
	var hazardousCautions = rowData.hazardous_cautions.split(" ");
	if (hazardousCautions.length > 1){
		var editHazardousCautionsVal = [];
		for (var i in hazardousCautions) {
			if (!hazardousCautions[i]) continue;
			editHazardousCautionsVal.push(
				$("#edit_hazardous_cautions").children("option").filter(function(){
					return this.innerText == hazardousCautions[i];
				}).val()
			);
		}
		$("#edit_hazardous_cautions").val(editHazardousCautionsVal).trigger("change");
	} else {
		$("#edit_hazardous_cautions").val("").trigger("change");
	}
