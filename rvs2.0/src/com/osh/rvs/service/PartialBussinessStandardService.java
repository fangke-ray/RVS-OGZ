	...
	public void update(SqlSessionManager conn, HttpServletRequest req, List<MsgInfo> errors) {
			...
			if (m.find()) {
				...
				if ("partial_bussiness_standard".equals(entity)) {
					...
					if ("spec_kind".equals(column)) {
						...
						formList.get(icounts).setOn_shelf(value[0]);
					...
					} else if ("collation_flg".equals(column)) {
						formList.get(icounts).setCollation_flg(value[0]);
					}
				}
			}
		}
		...
		if (errors.size() == 0) {
			...
			for (PartialBussinessStandardForm form : formList) {
				...
				// 上架
				String onShelf = form.getOn_shelf();
				String collationFlg = form.getCollation_flg();
				...
				if ("0".equals(boxCountFlg) && Double.valueOf(boxCount) <= 0) {
					...
					error.setErrmsg(ApplicationMessage.WARNING_MESSAGES.getMessage("validator.invalidParam.invalidMoreThanZero", CodeListUtils.getValue(PARTIAL_SPEC_KIND, form.getSpec_kind())
							+ "装箱数量"));
					errors.add(error);
				}
				...
				if (Double.valueOf(collation) <= 0) {
					MsgInfo error = new MsgInfo();
					error.setErrcode("validator.invalidParam.invalidMoreThanZero");
					error.setErrmsg(ApplicationMessage.WARNING_MESSAGES.getMessage("validator.invalidParam.invalidMoreThanZero", CodeListUtils.getValue(PARTIAL_SPEC_KIND, form.getSpec_kind()) + "核对"));
					errors.add(error);
				}

				if ("0".equals(collationFlg) && Double.valueOf(onShelf) <= 0) {
					MsgInfo error = new MsgInfo();
					error.setErrcode("validator.invalidParam.invalidMoreThanZero");
					error.setErrmsg(ApplicationMessage.WARNING_MESSAGES.getMessage("validator.invalidParam.invalidMoreThanZero", CodeListUtils.getValue(PARTIAL_SPEC_KIND, form.getSpec_kind()) + "上架"));
					errors.add(error);
				}
				...
				if (!CommonStringUtil.isEmpty(unpack) && Double.valueOf(unpack) <= 0) {
					MsgInfo error = new MsgInfo();
					error.setErrcode("validator.invalidParam.invalidMoreThanZero");
					error.setErrmsg(ApplicationMessage.WARNING_MESSAGES.getMessage("validator.invalidParam.invalidMoreThanZero", CodeListUtils.getValue(PARTIAL_SPEC_KIND, form.getSpec_kind()) + "分装"));
					errors.add(error);
				}
