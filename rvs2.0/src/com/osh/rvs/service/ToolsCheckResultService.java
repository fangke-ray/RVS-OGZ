...
public class ToolsCheckResultService {
	...
	public List<ToolsCheckResultForm> searchToolsCheckResult(ToolsCheckResultForm toolsCheckResultForm,SqlSession conn) {
		...
		JigCheckResultMapper dao = conn.getMapper(JigCheckResultMapper.class);
		
		JigCheckResultEntity toolsCheckResultEntity = new JigCheckResultEntity();
		//form-->bean
		...
		//检索后的数据
		List<JigCheckResultEntity> retultEntitys = null;
		
		JigCheckResultEntity returnEntity = null;
		
		Map<String,JigCheckResultEntity> map = new TreeMap<String,JigCheckResultEntity>();
	        ...
	        if (retultEntitys != null && retultEntitys.size() > 0) {
				if (i == 0) {//
					for (int j = 0; j < retultEntitys.size(); j++) {
						JigCheckResultEntity tempEntity = retultEntitys.get(j);
						returnEntity = new JigCheckResultEntity();
						returnEntity.setManage_id(tempEntity.getManage_id());
						returnEntity.setManage_code(tempEntity.getManage_code());
						returnEntity.setJig_no(tempEntity.getJig_no());
						returnEntity.setJig_name(tempEntity.getJig_name());
		...
		while (iter.hasNext()) {
			...
			JigCheckResultEntity mapEntity = map.get(key);
	...
	/**
	 * 治具点检结果--课室+工程+工位
	 * @param JigCheckResultEntity
	 * @param conn
	 * @return
	 */
	public ToolsCheckResultForm searchSectionLinePosition(JigCheckResultEntity toolsCheckResultEntity, SqlSession conn) {
		ToolsCheckResultForm toolsCheckResultForm = new ToolsCheckResultForm();
		JigCheckResultMapper dao = conn.getMapper(JigCheckResultMapper.class);
		//课室+工程+工位
		JigCheckResultEntity resultEntity = dao.searchSectionLinePosition(toolsCheckResultEntity);
	 ...
	 * @return
	 */
	public List<ToolsCheckResultForm> detailCheckResult(JigCheckResultEntity toolsCheckResultEntity,SqlSession conn){
		List<ToolsCheckResultForm> toolsCheckResultForms = new ArrayList<ToolsCheckResultForm>() ;
		JigCheckResultMapper dao = conn.getMapper(JigCheckResultMapper.class);
		...
		//设备工具点检记录
		List<JigCheckResultEntity> resultEntitys = dao.searchCheckResult(toolsCheckResultEntity);
